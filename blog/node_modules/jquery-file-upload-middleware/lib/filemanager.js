...
module.exports = function (middleware, options) {
    ...
    FileManager.prototype.getFiles = function (dir,callback) {
        if(!dir || dir=='undefined') dir = '';
        var files = {};
        var counter = 1;
        var finish = function () {
            if (!--counter)
                callback(files);  //{files:files}
        };

        fs.readdir(options.uploadDir()+dir, _.bind(function (err, list) {
            _.each(list, function (name) {
                var stats = fs.statSync(options.uploadDir() + dir + '/' + name);
                if (stats.isFile()) {
                    //files.push({name:name,path: options.uploadDir() + '/' + name}) ;
                    files[name] = {
                        path: options.uploadDir() + dir + '/' + name
                    };
